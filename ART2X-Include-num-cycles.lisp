;;****************************** ART2.lisp **************************
;;
;;*** THIS IS A FILE I STARTED TO CHANGE ALL THE ART ARRAYS
;;   TO EACH INCLUDE A CYCLE-NUM (MAX NUM-CYCLES) DIMENSION
;;   AS THE FIRST DIMENSION OF EACH ARRAY 
;;   ADVANTAGE: KEEPS FROM REDUNDANTLY COPYING THE DATA 
;;   TO NEW ACCUMULATION ARRAYS AT END OF EACH CYCLE
;;   DISADVANTAGE: CAN'T CONTROL WHICH ARRAYS ARE ACCUMULATED (ALL DATA IS KEPT) MAYBE CAUSE MEMORY PROBLEM LATER.
;;   ALSO, IT IS MORE COMPLEX TO WRITE THE CODE
;;
...
(defun ART2-manager (num-cycles ninputs noutputs test-pattern-lists
  ...
  (let
    ...
    ( initNetwork num-cycles nInputs nOutputs)

    ;;STEP 4: RUN ART2 -- CALCULATE THE VALUES FOR EACH CYCLE 
    ;; ON EACH FIELD AND CELL
    (setf *learned-patterns
          (learnPatterns num-Cycles nInputs nOutputs test-pattern-lists))  
  ...
  *ART2-INSTANCE nil
  ...
  (let 
    ...
    (dolist (spec-list symbol-spec-lists)
      ;;changed to add num-cycles
     (setf dims-list (append (list num-cycles) (list (second (second spec-list))))
...
;;(setf xy4 (second (car    '((X-Activity ((0 1))  ) (X-Activity ((1 1))  ) (X-Activity ((2 1))  ) (X-Activity ((3 1))  ) (X-Activity ((4 1))  )))))
    ...
    ;;  (afout 'out (format nil "1 PRExxx In new-symbol-type-spec-list-of-lists, spec-list1= ~A~% array-dims1= ~A~%ncells1= ~A  n-dims1= ~a~% dims-lists1= ~a~%" spec-list1 array-dims1 ncells1  n-dims1 dims-lists1 ))
      ...
      ;;mmm is this OK? 
      (setf array-dims1 (append (list numCycles) (make-list n-dims1 :initial-element ncells1)))
...
(defun findLargestOutput (cycle-num nOutputs 
                                    &aux (maxIndex 0) mVal)
  (setf mVal (aref *y-output cycle-num 0)) ;;added
  (dotimes (j nOutputs)
    (if (and
         (> (aref *y-output cycle-num j) mVal)
         (not (aref *reset cycle-num j)))
      (setq mVal (aref *y-output cycle-num j)
          maxIndex j)))
...
(defun g (index cycle-num nOutputs  
                &aux j mVal)   ;; (maxIndex (findLargestOutput)))
  (let
      ((maxIndex (findLargestOutput cycle-num nOutputs))
       )
  (if (and
       (equal index maxIndex)
       (not (aref *reset cycle-num maxIndex))
       (> (aref *y-output cycle-num maxIndex) resetThreshold))
    d
    0.0)))
...
(defun testReset (num-cycles nInputs nOutputs 
                             &aux (res 0.0) (norm1 0.0) (norm2 0.0)  
                             (n1 (+ (L2NORM p nInputs) e)) n2 temp)
  (if (and
       (> n1 0.2)
       (not skipReset))
      (if (> learningCycleCounter 1)
          (if (> (aref *y-output (findLargestOutput
;;sss
                                  learningCycleCounter nOutputs)) 0.25)
...
; Zero activations:
;;mmm
(defun zeroActivations (num-cycles nnInputs nOutputs)
  (dotimes (n num-cycles)
    (dotimes (i nInputs)
      (afout 'out (format nil  "setting aref w-p ~a ~a => 0 ~%"n  i))
      ;;is this needed??  (setf (aref *w i) 0.0) 
      (setf (aref *x-activity n i) 0.0)
      (setf (aref *v n i) 0.0)
      (setf (aref *r n i) 0.0)
      (setf (aref *u n i) 0.0)
      (setf (aref *q n i) 0.0)
      (setf (aref *p n i) 0.0))
    (dotimes (i nOutputs)
      (setf (aref *y-output n i) 0)
      (afout 'out (format nil "setting aref ~a ~a ~a => 0 ~%" n *y-output i))
      (setf (aref *reset n i) 0)
      (setf (aref *reset-cntr n i) 0))))

; Set up a learning pattern in the input neurons:

(defun setPattern (pl num-cycles nInputs nOutputs &aux (len (length pl)))
  (if (not (equal len nInputs))
      (print (list "Error in setPattern input:" pl))
    (progn
      (setq learningCycleCounter 0)
      (zeroActivations num-cycles nInputs nOutputs)
      (dotimes (n num-cycles)
        (dotimes ( i len)
          (setf (aref *input n i) (+ (pop pl) (frandom -0.08 0.08))))))))
...
(defun initNetwork (num-cycles nInputs nOutputs)
  "In ART2.lisp,   I added num-cycles to the arrays"
  (zeroActivations num-cycles nInputs nOutputs)
  (dotimes (n num-cycles)
    (dotimes (j nOutputs)
      (progn
        (dotimes (i nInputs)
          (setf
           (aref *wUp n i j) (frandom 0.05 0.1)
           (aref *wdn n j i) (frandom 0.01 0.03))
          (afout 'out (format nil "wUp ~a ~a ~a => ~a ~%"n  i j (aref *wUp n i j)))
          (afout 'out (format nil  "wDn ~a ~a ~a => ~a ~%"n  j i (aref *wdn n j i)))        
          )
...
(defun learnPatterns (num-cycles nInputs nOutputs testPatterns) 
